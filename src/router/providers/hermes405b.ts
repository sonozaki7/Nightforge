...
const logger = pino({ name: "nightforge-hermes405b" });

const DEFAULT_MODEL = "nousresearch/hermes-4-405b";
const DEFAULT_BASE_URL = "https://openrouter.ai/api/v1";

// OpenRouter pricing for Hermes 4 405B (zero censorship, maximum capability)
const COST_PER_MILLION_INPUT = 1.0;
const COST_PER_MILLION_OUTPUT = 3.0;
...
export function createHermes405bProvider(config: ProviderConfig): Provider {
  ...
  return {
    ...
    async generate(
      ...
      logger.info({ model }, "Calling OpenRouter API (Hermes 405B)");
      ...
      logger.info(
        ...
        "OpenRouter API response received (Hermes 405B)"
      );
