...
const logger = pino({ name: "nightforge-composer" });

const DEFAULT_MODEL = "composer-2.5-standard";
const DEFAULT_BASE_URL = "https://api.cursor.com/v1";

const COST_PER_MILLION_INPUT = 0.5;
const COST_PER_MILLION_OUTPUT = 2.0;
...
export function createComposerProvider(config: ProviderConfig): Provider {
  ...
  return {
    ...
    async generate(
      ...
      logger.info({ model }, "Calling Composer API");
      ...
      logger.info(
        ...
        "Composer API response received"
      );
